...
在 `frontend/index.html` 中配置高德地图API密钥：
...
5. 复制 Key 和 安全密钥（_AMapSecurityConfig.securityJsCode）到上述代码中

**注意事项：**
- 确保您的 Key 已配置正确的域名白名单（本地开发可以使用 `localhost`）
- 安全密钥在2021年12月后创建的应用必须配置
- 生产环境需要配置实际的域名
